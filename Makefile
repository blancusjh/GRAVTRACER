...
SRCS = src/kind_params.f90 src/kerr_metric.f90 src/q_metric.f90 \
       src/tabulated_metric.f90 src/spacetime.f90 src/geodesic_eom.f90 src/rk_integrators.f90 \
...
core:
	FFLAGS="-O3 -fopenmp" $(F2PY) -c $(SRCS) -m _core --dep openmp \
	    only: metric_cov metric_contra get_horizon get_isco camera_init \
	          ...
	          trace_to_plane trace_bundle_to_plane set_metric_table render_geometry render_crossings :
