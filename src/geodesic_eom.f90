MODULE GEODESIC_EOM
   ...
   ! p_t and p_phi are conserved and passed as parameters.
   USE KIND_PARAMS, ONLY: WP
   USE KERR_METRIC, ONLY: METRIC_CONTRA
   IMPLICIT NONE
   PRIVATE
...
CONTAINS
   ...
   PURE FUNCTION HAMILTONIAN_CONSTRAINT(A, PT, PPHI, Y) RESULT(H)
      ! H must remain zero along a null geodesic (eq. 3); its numerical
      ! value is the error monitor of eq. (12).
      REAL(WP), INTENT(IN) :: A, PT, PPHI, Y(6)
      REAL(WP) :: H
      REAL(WP) :: GU(5), DGR(5), DGT(5)
