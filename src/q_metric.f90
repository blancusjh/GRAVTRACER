...
CONTAINS
   ...
   PURE SUBROUTINE QMETRIC_COV(Q, R, THETA, GD)
      ! The fractional powers f^q, f^(1+q), h^alpha are built from two
      ! shared logs (pow = exp(y*log(x)) under the hood; sharing the
      ! logs saves the redundant transcendental calls).
      REAL(WP), INTENT(IN)  :: Q, R, THETA
      REAL(WP), INTENT(OUT) :: GD(5)
      REAL(WP) :: F, H, S2, ALPHA, FQ, F1Q, HA
   ...
   PURE SUBROUTINE QMETRIC_CONTRA(Q, R, THETA, GU, DGU_DR, DGU_DTH)
      ...
      ! shared fractional powers: 2 logs + 2 exps instead of 4 pows
      FQ = EXP(Q*LOG(F))          ! f^q
      F1Q = F*FQ                  ! f^(1+q)
      HA = EXP(ALPHA*LOG(H))      ! h^alpha
