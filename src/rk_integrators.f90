...
CONTAINS
   ...
   PURE SUBROUTINE RK_EMBEDDED_STEP(METHOD, A, PT, PPHI, Y, H, YNEW, ERRV)
      INTEGER, INTENT(IN)   :: METHOD
      REAL(WP), INTENT(IN)  :: A, PT, PPHI, Y(6), H
      REAL(WP), INTENT(OUT) :: YNEW(6), ERRV(6)

      SELECT CASE (METHOD)
      CASE (METHOD_RKCK45)
         CALL STEP_RKCK45(A, PT, PPHI, Y, H, YNEW, ERRV)
      CASE (METHOD_RKF45)
         CALL STEP_RKF45(A, PT, PPHI, Y, H, YNEW, ERRV)
      CASE DEFAULT
         CALL STEP_RKDP45(A, PT, PPHI, Y, H, YNEW, ERRV)
      END SELECT
   END SUBROUTINE RK_EMBEDDED_STEP

   PURE SUBROUTINE STEP_RKDP45(A, PT, PPHI, Y, H, YNEW, ERRV)
      REAL(WP), INTENT(IN)  :: A, PT, PPHI, Y(6), H
      REAL(WP), INTENT(OUT) :: YNEW(6), ERRV(6)
      REAL(WP) :: K1(6), K2(6), K3(6), K4(6), K5(6), K6(6), K7(6)

      CALL GEODESIC_RHS(A, PT, PPHI, Y, K1)
      CALL GEODESIC_RHS(A, PT, PPHI, Y + H*(0.2_WP*K1), K2)
      CALL GEODESIC_RHS(A, PT, PPHI, Y + H*((3.0_WP/40.0_WP)*K1 + &
           (9.0_WP/40.0_WP)*K2), K3)
      CALL GEODESIC_RHS(A, PT, PPHI, Y + H*((44.0_WP/45.0_WP)*K1 - &
           (56.0_WP/15.0_WP)*K2 + (32.0_WP/9.0_WP)*K3), K4)
      CALL GEODESIC_RHS(A, PT, PPHI, Y + H*((19372.0_WP/6561.0_WP)*K1 - &
           (25360.0_WP/2187.0_WP)*K2 + (64448.0_WP/6561.0_WP)*K3 - &
           (212.0_WP/729.0_WP)*K4), K5)
      CALL GEODESIC_RHS(A, PT, PPHI, Y + H*((9017.0_WP/3168.0_WP)*K1 - &
      ...
      CALL GEODESIC_RHS(A, PT, PPHI, YNEW, K7)
   ...
   PURE SUBROUTINE STEP_RKCK45(A, PT, PPHI, Y, H, YNEW, ERRV)
      REAL(WP), INTENT(IN)  :: A, PT, PPHI, Y(6), H
      REAL(WP), INTENT(OUT) :: YNEW(6), ERRV(6)
      ...
      CALL GEODESIC_RHS(A, PT, PPHI, Y + H*((3.0_WP/40.0_WP)*K1 + &
           (9.0_WP/40.0_WP)*K2), K3)
      CALL GEODESIC_RHS(A, PT, PPHI, Y + H*(0.3_WP*K1 - 0.9_WP*K2 + &
           1.2_WP*K3), K4)
      CALL GEODESIC_RHS(A, PT, PPHI, Y + H*((-11.0_WP/54.0_WP)*K1 + &
           2.5_WP*K2 - (70.0_WP/27.0_WP)*K3 + (35.0_WP/27.0_WP)*K4), K5)
      CALL GEODESIC_RHS(A, PT, PPHI, Y + H*((1631.0_WP/55296.0_WP)*K1 + &
   ...
   PURE SUBROUTINE STEP_RKF45(A, PT, PPHI, Y, H, YNEW, ERRV)
      REAL(WP), INTENT(IN)  :: A, PT, PPHI, Y(6), H
      REAL(WP), INTENT(OUT) :: YNEW(6), ERRV(6)
      REAL(WP) :: K1(6), K2(6), K3(6), K4(6), K5(6), K6(6)

      CALL GEODESIC_RHS(A, PT, PPHI, Y, K1)
      CALL GEODESIC_RHS(A, PT, PPHI, Y + H*(0.25_WP*K1), K2)
      CALL GEODESIC_RHS(A, PT, PPHI, Y + H*((3.0_WP/32.0_WP)*K1 + &
           (9.0_WP/32.0_WP)*K2), K3)
      CALL GEODESIC_RHS(A, PT, PPHI, Y + H*((1932.0_WP/2197.0_WP)*K1 - &
           (7200.0_WP/2197.0_WP)*K2 + (7296.0_WP/2197.0_WP)*K3), K4)
      CALL GEODESIC_RHS(A, PT, PPHI, Y + H*((439.0_WP/216.0_WP)*K1 - &
           8.0_WP*K2 + (3680.0_WP/513.0_WP)*K3 - &
           (845.0_WP/4104.0_WP)*K4), K5)
      CALL GEODESIC_RHS(A, PT, PPHI, Y + H*((-8.0_WP/27.0_WP)*K1 + &
